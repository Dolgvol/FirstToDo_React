...
const NoteRow = ({note, onView, onEdit, onDelete, onToggleStatus}) => {
    return (
        <div className={`item-row ${note.active ? 'active' : 'archived'}`}>
            ...
            <div className="item-category item-cell">{labels[note.category]}</div>
            ...
            <div className="item-managment">
                ...
                <a className='archive-current managment-icon'>
                    <FontAwesomeIcon icon={note.active ? faDownload : faUpload} onClick={() => onToggleStatus()}/>
                </a>
